...
const buildUrl = (...urlComponents) => {
    return (urlComponents
        // Trim leading & trailing slashes
        .map((component) => String(component).replace(/^\/|\/$/g, ""))
        .join("/"));
};
const slackRunner = async ({ ciProvider, vcsRoot, reportDir, videoDir, screenshotDir, customUrl = "", onlyFailed = false, customText = "", }) => {
    try {
...
const getHTMLReportFilename = async (reportDir) => {
    const reportHTMLFullPath = await globby(isWin
...
const getTestReportStatus = async (reportDir) => {
    const reportFile = await globby(isWin
        ? path.resolve(reportDir).replace(/\\/g, "/")
        : path.resolve(reportDir), {
        expandDirectories: {
            files: ["*"],
...
const getVideoLinks = async ({ artefactUrl, videosDir, }) => {
    if (!artefactUrl) {
        ...
        const videos = await globby(isWin
        ...
        if (videos.length === 0) {
            ...
            const videoLinks = await Promise.all(videos.map((videoObject) => {
                ...
                return `<${buildUrl(videosURL, videosDir, path.relative(videosDir, videoObject))}|Video:- ${trimmedVideoFilename}>\n`;
            }));
...
const getScreenshotLinks = async ({ artefactUrl, screenshotDir, }) => {
    if (!artefactUrl) {
        ...
        const screenshots = await globby(isWin
        ...
        if (screenshots.length === 0) {
            ...
            const screenshotLinks = await Promise.all(screenshots.map((screenshotObject) => {
                ...
                return `<${buildUrl(screenshotURL, screenshotDir, path.relative(screenshotDir, screenshotObject))}|Screenshot:- ${trimmedScreenshotFilename}>\n`;
            }));
            return screenshotLinks.join("");
        }
    }
};
const buildHTMLReportURL = async ({ reportDir, artefactUrl, }) => {
...
const resolveCIProvider = async (ciProvider) => {
    let { CI_SHA1, CI_BRANCH, CI_USERNAME, CI_BUILD_URL, CI_BUILD_NUM, CI_PULL_REQUEST, CI_PROJECT_REPONAME, CI_PROJECT_USERNAME, JOB_NAME, CIRCLE_PROJECT_ID, CIRCLE_WORKFLOW_JOB_ID, } = process.env;
